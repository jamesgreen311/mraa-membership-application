   ...
   return {
      boardmembers: {
         ...
         schema: {
            ...
            email: "d", // position will give the row number for officers email, etc
            firstName: "c",
            lastName: "b",
         },
      },
...
function getMemberByPosition(position) {
   ...
   const positionList = bm
      .getRange(schema.position + bm.getLastRow())
      .getDisplayValues()
   ...
   if (row >= 0) {
      ...
      member.firstName = bm
         .getRange(`${schema.firstName}${row}`)
         .getDisplayValue()
      member.lastName = bm
         .getRange(`${schema.lastName}${row}`)
         .getDisplayValue()
   }
   return member
}
...
function getBoardMembershipChair() {
   return getMemberByPosition("Membership")
}
